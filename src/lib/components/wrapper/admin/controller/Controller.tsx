...
export type ControllerProps = {
  onValueChange?: (
    x: number,
    y: number,
    executeOnCustomChanges: boolean,
  ) => void;
};

const Controller = forwardRef<ControllerRef, ControllerProps>(
  function Controller({ onValueChange }, ref) {
    ...
    function onDragHandler() {
      ...
      for (const piece of transformPieces) {
        if (piece.startsWith("translateX")) {
          transformX = parseFloat(piece.substring(
            splitBeforeLength,
            piece.length - 3,
          ));
        } else if (piece.startsWith("translateY")) {
          transformY = parseFloat(piece.substring(
            splitBeforeLength,
            piece.length - 3,
          ));
        }
      ...
      `}
        ...
        <div
          ...
          style={{
            ...
            borderRadius: circler ? parentSize * .28 : parentSize * .06,
